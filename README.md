...
## Generate from a UI image

In **AI Source**, choose a PNG, JPEG, or WebP UI screenshot (up to 10 MB), enter an OpenAI API key, then choose **Generate image → Figma with Terra**. The plugin reads the image as a data URL, sends it with the current file's component/token catalog to the OpenAI Responses API, validates the returned JSON, and imports editable Figma layers. It does not place the screenshot itself on the canvas. The source image is held only in memory by the plugin and is not stored in the Figma document or plugin storage.

The visual reconstruction is necessarily approximate: photographs, illustrations, logos, icons without a matching local component, and small typography details may require manual refinement. Use a high-resolution, uncropped UI screenshot for the best result.
