type PluginMessage = { type: 'ui-ready' } | { type: 'export-catalog' } | { type: 'export-selection' } | { type: 'import-json'; data: unknown } | { type: 'generate-wireframe'; apiKey: string; wireframe: string; prompt: string } | { type: 'generate-image'; apiKey: string; imageDataUrl: string; imageWidth: number; imageHeight: number; prompt: string } | { type: 'cancel' };
type CatalogEntry = {
  name: string;
...
figma.showUI(__html__, { width: 480, height: 820, themeColors: true });
...
function parseGeneratedImport(response: OpenAIResponse): ImportPayload {
  if (response.status === 'incomplete') {
    const reason = typeof response.incomplete_details?.reason === 'string' ? response.incomplete_details.reason : 'unknown reason';
    throw new Error(`The model response was incomplete (${reason}). Try a simpler screenshot or run Generate again.`);
  }
  let parsed: unknown;
  try {
...
async function generateImageJson(message: Extract<PluginMessage, { type: 'generate-image' }>): Promise<ImportPayload> {
  const apiKey = message.apiKey.trim();
  if (!apiKey) throw new Error('Enter an OpenAI API key.');
  if (!message.prompt.trim()) throw new Error('The bundled image prompt is unavailable. Run npm run build again.');
  if (!isSupportedImageDataUrl(message.imageDataUrl)) throw new Error('Choose a valid PNG, JPEG, or WebP image.');
  if (message.imageDataUrl.length > 14 * 1024 * 1024) throw new Error('The selected image is too large. Choose an image smaller than 10 MB.');
  if (!Number.isSafeInteger(message.imageWidth) || !Number.isSafeInteger(message.imageHeight) || message.imageWidth < 1 || message.imageHeight < 1) {
    ...
    body: JSON.stringify({
      ...
      input: [{
        ...
        content: [
          {
            type: 'input_text',
            text: `Return exactly one valid JSON object that can be imported by the Figma plugin.\n\nThe attached image is the UI reference. Its exact canvas size is ${message.imageWidth} × ${message.imageHeight} pixels; use those dimensions for the root frame.\n\nFIGMA DESIGN SYSTEM CATALOG (current file data):\n${JSON.stringify(catalog)}`,
          },
          { type: 'input_image', image_url: message.imageDataUrl, detail: 'high' },
        ],
      }],
    }),
  });
  ...
  return parseGeneratedImport(response);
}
...
function componentFor(name: string, variant?: string): ComponentNode | null {
  ...
  const direct = nodes.find((node) => node.type === 'COMPONENT' && node.name === name)
    ?? nodes.find((node) => node.type === 'COMPONENT' && normalized(node.name) === normalizedName);
  if (direct?.type === 'COMPONENT') return direct;
  const set = nodes.find((node) => node.type === 'COMPONENT_SET' && node.name === name)
    ?? nodes.find((node) => node.type === 'COMPONENT_SET' && normalized(node.name) === normalizedName);
  ...
  const match = variants.find((child) => child.name === ordered)
    ...
    ?? (Object.keys(requestedProperties).length ? variants.find((child) => {
      const available = variantValues(child);
      return Object.entries(requestedProperties).every(([property, value]) => Object.entries(available)
        .some(([availableProperty, availableValue]) => normalized(availableProperty) === normalized(property) && normalized(availableValue) === normalized(value)));
    }) : undefined)
...
function namedResource<T extends { name: string }>(resources: readonly T[], name?: string): T | undefined {
  ...
  return resources.find((resource) => resource.name === name)
    ?? resources.find((resource) => normalized(resource.name) === normalized(name));
}
...
figma.ui.onmessage = async (msg: PluginMessage) => {
  try {
    ...
    if (msg.type === 'generate-image') {
      const data = await generateImageJson(msg);
      return figma.ui.postMessage({ type: 'image-generated', data });
    }
